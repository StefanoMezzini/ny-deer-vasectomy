...
p_speed <- readRDS('models/predictions/speed-preds_mu.rds') %>%
  ggplot() +
  facet_grid(sex ~ .) +
  geom_vline(xintercept = REF_DATES[c(1, 3)], col = 'red') +
  geom_ribbon(aes(date, ymin = lwr_95, ymax = upr_95, fill = site),
...
p_diff <- readRDS('models/predictions/diffusion-preds_mu.rds') %>%
  ggplot() +
  facet_grid(sex ~ .) +
  geom_vline(xintercept = REF_DATES[c(1, 3)], col = 'red') +
  geom_ribbon(aes(date, ymin = lwr_95, ymax = upr_95, fill = site),
...
p_exc <- readRDS('models/predictions/density-preds_mu.rds') %>%
  ggplot() +
  facet_grid(sex ~ .) +
  geom_vline(xintercept = REF_DATES[c(1, 3)], col = 'red') +
  geom_ribbon(aes(date, ymin = lwr_95, ymax = upr_95, fill = site),
