...
# Function window_hr
# Adjusted - based on Stefano Mezzini: https://github.com/QuantitativeEcologyLab/hr-resource-stoch/blob/main/functions/window_hr.R
# ------------------------------------------------------------------------------
...
#                    speed = ctmm::speed(object = d, CTMM = model[[1]], units = FALSE, cores = cores) %>%
#                      list(),
#                    
#                    #print("Speed estimated!"),
#                    
...
#                    
#                    d.length = max(d$timestamp) - min(d$timestamp),
#                    d.length.int = as.integer(max(d$timestamp) - min(d$timestamp)), # length(unique(cut(d$timestamp, breaks = "day"))),
#                    
...
#                                '-days-dt-', dt / (1 %#% 'day'),
#                                '-days.png')) %>%
#       ggsave(plot = plt, units = 'in', width = 9, height = 3,
#              dpi = 600, bg = 'white')
#   } else {
#     print(plt)
#   }
...
#   if (! is.null(fig_path)) {
#     
#     print("plot the variogram and model fitted for each moving window")
# 
#     png(file.path(fig_path, paste0(tel@info['identity'],
#                                    '-window-', window / (1 %#% 'day'),
#                                    '-days-dt-', dt / (1 %#% 'day'),
#                                    '-days_var_mf.png')), width = 60, height = 70, units='cm', res = 600)
#     
#     par(mfrow = c(5, 5))
#     for(i in 1:dim(out)[1]){
#       plot(out[i, "variogram"][[1]], out[i, "model"][[1]])
#     }
#     dev.off()
#     par(mfrow = c(1, 1))
#     
#     
#     prin("plot the akde for each moving window")
#     
#     png(file.path(fig_path, paste0(tel@info['identity'],
#                                    '-window-', window / (1 %#% 'day'),
#                                    '-days-dt-', dt / (1 %#% 'day'),
#                                    '-days_akdes.png')), width = 65, height = 65, units='cm', res = 600)
#     
#     par(mfrow = c(5, 5))
#     for(i in 1:dim(out)[1]){
#       plot(out[i, "akde"][[1]])
...
# Author: Petar Bursac
# Date: 05.01.2024.
# Description: Function to call window_hr and go trough ids and years
...
window_hr_parallel <- function(tel, window, dt, projection, full_ud = NULL,
                      fig_path = NULL, rds_path = NULL, cores = 1,
                      weights = FALSE, plot_50_q = FALSE) {
  ...
  for(wind in 1:dim(times.df)[1]){ # dim(times.df)[1]
    ...
    print(paste0("Analyzing dataset ", wind, " of ", dim(times.df)[1], "."))
  ...
  # models1 <- models %>% slice(rep(1:10, each = 5))
  ...
  if (! is.null(fig_path)) {
    ...
    par(mfrow = c(round(dim(out)[1]/2, 0), round(dim(out)[1]/2, 0)))
    for(i in 1:dim(out)[1]){
      plot(out[i, "variogram"][[1]], out[i, "model"][[1]])
    }
    dev.off()
...
# plot_50_q = FALSE
# weights = FALSE
# parallelization - with do parallel

# ------------------------------------------------------------------------------


window_hr_do_parallel <- function(tel, window, dt, projection, full_ud = NULL,
                                  fig_path = NULL, rds_path = NULL, cores = 1,
                                  weights = FALSE, plot_50_q = FALSE) {
  ...
  models.list <- foreach(j = 1:dim(times.df)[1], .packages = c("dplyr", "magrittr", "ctmm")) %dopar% { # .export = c("extract_hr", "extract_speed_gauss", "extract_diffusion", "extract_speed_model", "extract_tau_p", "extract_tau_v")
                           ...
                           # print(paste0("Analyzing dataset ", j, " of ", dim(times.df)[1], "."))
  ...
  #   
  #   print(plot(models.list[[i]]$variogram, models.list[[i]]$model))
  # }
  
  # 5 models: 11:16 - 11:28 --- 12min 
  # 3.6h for 90 models - 1 individual
  # 7x times faster
  # 123 individuals (both years: 63 Year 1 and 60 Year 2)
  # 442.8h - 18.5 days
